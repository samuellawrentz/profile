...
- **Hero image workflow**: Generate new blog art with `bun run gen:image "<prompt>" --output ./src/content/blog/<post-dir> --name header`. Keep prompts visual and explicitly ask for **no text** in the image. The model (`gemini-2.5-flash-image`) has no free tier: an unbilled key returns 429 with `limit: 0`, so `GOOGLE_AI_API_KEY` must be from a billed project. `image-gen` is a `file:` dependency, so `node_modules/image-gen` is a copy; after editing `packages/image-gen/src`, run `bun install` or `gen:image` keeps running the old code. If `gen:image` dies with `Cannot find package 'image-gen'`, `node_modules` is stale: run `bun install` first.
